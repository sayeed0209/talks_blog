...
<div class="main flex-grow-1 pt-4 px-5 d-flex justify-content-center justify-content-lg-start" :class="{dark}">
    <div class="mr-5 d-none d-lg-block">
        <div class="main__user sticky-top pt-3" id="side">
            <img v-bind:src="'http://localhost:8000/images/profile/'+ user.picture_filename"  class="main__user--img"/>
            <h5 class="mt-3">{{user.name}} {{user.lastname}}</h5>
            ...
            <div class="mt-3">
                ...
                <p class="text-wrap">{{user.description}}</p>
            </div>
        </div>
    </div>
...
export default {
    ...
    created(){
        ...
        this.user_id = user
        // const url = `http://localhost:8000/post/list/user/${user}/1`;
        ...
        axios.get(`http://localhost:8000/get/followers/${user}`)
        .then(res => {
